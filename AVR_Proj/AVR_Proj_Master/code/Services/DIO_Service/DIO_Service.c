...
DIO_Service_PinCfg_t DIO_Service_Pin_data[DIO_Service_PIN_COUNT] = {DIO_Service_Pin_data_INIT};
#endif
...
void DIO_Service_Init(void)
{
    ...
    for(pin_id=0; pin_id<DIO_Service_PIN_COUNT; pin_id++)
    {
        DIO_Service_Pin_data[pin_id].status = DIO_Init_PIN(pin_id, DIO_Service_Pin_data[pin_id].pin_type);

        if(DIO_Service_PIN_OUTPUT == DIO_Service_Pin_data[pin_id].pin_type)
        {
...
Func_ReturnType DIO_Service_Set_Pin(uint8 pin_id, uint8 pin_type)
{
    ...
    if(RET_OK == DIO_Service_Pin_data[pin_id].status)
    {
        ...
        {
            if(DIO_Service_PIN_INPUT == pin_type)
            {
                DIO_SET_PIN_AS_INPUT(pin_id);
                returnL = RET_OK;
            }
            else if (DIO_Service_PIN_OUTPUT == pin_type)
            {
...
#if DIO_Service_PIN_COUNT > 0
    if((RET_OK == DIO_Service_Pin_data[pin_id].status) && (DIO_Service_PIN_INPUT == DIO_Service_Pin_data[pin_id].pin_type))
    {
...
#if DIO_Service_PIN_COUNT > 0
    if( (RET_OK == DIO_Service_Pin_data[pin_id].status) && (DIO_Service_PIN_OUTPUT == DIO_Service_Pin_data[pin_id].pin_type) )
    {
        if(pin_state != DIO_Service_Pin_data[pin_id].pin_state)
        {
            if(DIO_Service_PIN_LOW == pin_state)
            {
                DIO_SET_PIN_LOW(pin_id);
                DIO_Service_Pin_data[pin_id].pin_state = pin_state;
                returnL = RET_OK;
            }
            else if (DIO_Service_PIN_HIGH == pin_state)
            {
...
Func_ReturnType DIO_Service_Toggle_Pin(uint8 pin_id)
{
    ...
    if((RET_OK == DIO_Service_Pin_data[pin_id].status) && (DIO_Service_PIN_OUTPUT == DIO_Service_Pin_data[pin_id].pin_type))
    {
        if(DIO_Service_PIN_LOW == DIO_Service_Pin_data[pin_id].pin_state)
        {
            DIO_Service_Write_Pin(pin_id, DIO_Service_PIN_HIGH);
            returnL = RET_OK;
        }
        else if(DIO_Service_PIN_HIGH == DIO_Service_Pin_data[pin_id].pin_state)
        {
            DIO_Service_Write_Pin(pin_id, DIO_Service_PIN_LOW);
            returnL = RET_OK;
        }
        else
        {
